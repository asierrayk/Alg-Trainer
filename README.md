...
- The cube never changes size. The bands above and below it are a fixed height, so revealing the solution, a long algorithm or a short one, and switching between the three views all leave the cube exactly where it was. A sticker is the same size in every view.
- Top bar - the algset and Go in order sit next to the connect button and the settings gear, because those are the two things worth changing mid-session.
- Touch - **tap** the screen for the algorithm, **swipe left** for the next case (swiping past the newest one gives you a new scramble) and **swipe right** to go back.
- Guided algorithms - once the solution is up, the moves you have already made grey out and the next one is highlighted, the way csTimer guides you through a scramble. A turn that is not part of the algorithm shows in red with the move needed to undo it. It follows you whether the moves come from a smart cube, the keyboard or the virtual cube, and it copes with rotations and wide moves - progress is tracked by the state of the cube, not by matching the moves you type.
...
- Pressing spacebar, or tapping the screen, shows you the algorithm you should use. No more searching for algs in pdf documents!
